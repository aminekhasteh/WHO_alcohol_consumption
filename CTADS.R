...
row_creator_30 <- function(age_min = age_min, increment, 
  ...
  ####################CDTL = 30#########################
  ######################################################
  # N is the total number of people between each age interval
  N <- length(tmp_tmp_dat1$DVAGE)
  # a1==ALC_20: Have had a drink
  N_all <- length(tmp_tmp_dat1[which((tmp_tmp_dat1$ALC_20 == 1)),]$DVAGE) # The sample size based on which "cda" (and "fda" and "laa") is calculated
  # ALC_70: Number of drinks consumed - 30 days
  tmp_tmp_dat2 <- tmp_tmp_dat1[which((tmp_tmp_dat1$ALC_70 != 96)&
                                       (tmp_tmp_dat1$ALC_70 != 97)&
                                       (tmp_tmp_dat1$ALC_70 != 98)&
                                       (tmp_tmp_dat1$ALC_70 != 99)),]
  N1 <- length(tmp_tmp_dat2$DVAGE)
  print(paste0('Number of those who have reported drinking at least 1x in the last month is ',
               N1))
  if(N > 0){ # Changing N_all-->N since 11,904 individuals selected 6 (valid skip) for ALC_Q20
    cdtl <- 30 # last 30 days
    cda <- 100*(N1/N)
    cdase <- 100 * sqrt(((N1/N)*(1-N1/N))/N)
    #No ifnormation available to calculate fdtl, fda,fdase: Those who have not consumed alcohol in the last 1 year but have consumed alcohol before then
